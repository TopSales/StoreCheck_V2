...
namespace ZPF
{
   class PhotosPage : PageEx
   {
      ...
      public PhotosPage(string PhotoTitle, string SubTitle)
      {
         ...
         //g.Add(new MagasinView(), 0, 0);      // entête info 
         g.Add(new InterventionView(), 0, 0);   // entête info 

         pv = new PhotoView() { Text = (string.IsNullOrEmpty(PhotoTitle) ? "l'intervention" : SubTitle) };
         g.Add(pv, 0, 1);   // entête info 
      ...
      public async Task<string> OnProcessPhoto(string FileName, FileResult file)
      {
         ...
         else
         {
            ...
            FileNameTN = ZPF.XF.FileIO.CleanPath(FileNameTN);
            ...
            try
            {
               ...
               {
                  if ( ZPF.XF.FileIO.WriteStream(outputStream, FileNameTN))
                  {
