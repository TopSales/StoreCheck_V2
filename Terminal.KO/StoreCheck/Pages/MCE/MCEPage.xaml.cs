...
namespace StoreCheck.Pages
{
   ...
   public partial class MCEPage : Page_Base
   {
      ...
      private void Mce_OnMessage(MCEViewModel sender, string Message)
      {
         DoIt.OnMainThread(async () =>
         {
            await DisplayAlertT("StoreCheck", Message, "ok");
         });
      }
      ...
      protected override async void OnAppearing()
      {
         ...
         try
         {
            if (OnAppearing_First)
            {
               ...
               {
                  await DisplayAlertT("StoreCheck", "Error while loading the questionnaire!", "ok");
               };
            };
         }
         catch (Exception ex)
         {
            await DisplayAlertT("StoreCheck", "Error while loading the questionnaire!" + Environment.NewLine + ex.Message, "ok");
         };
      }
