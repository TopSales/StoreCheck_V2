...
namespace ZPF
{
   public class SyncViewModel : BaseViewModel
   {
      ...
      //public async Task<bool> SyncFamilies()
      //{
      //   LastError = "";

      //   //var list = await wsHelperwGet<EAN_Family[]>($@"/Familles/List/{MainViewModel.Current.Config.LastSynchro.ToString("dd.MM.yyyy HH:mm")}", Zipped: true);
      //   var list = await wsHelperwGet<EAN_Family[]>($@"/Familles/List/{DateTime.MinValue.ToString("dd.MM.yyyy HH:mm")}", Zipped: true);

      //   if (list != null)
      //   {
      //      MainViewModel.Current.SetLocalFamilies(list.ToList());
      //      return true;
      //   }
      //   else
      //   {
      //      LastError = T("SyncFamilies: Problem when communicating with the server:") + Environment.NewLine + LastError + Environment.NewLine + wsHelper.LastError;
      //      BackboneViewModel.Current.MessageBox(LastError);
      //      return false;
      //   };
      //}
      ...
      private async Task<bool> UploadInterventions()
      {
         ...
         foreach (var interv in l)
         {
            ...
            //try
            //{
            //   var json = System.IO.File.ReadAllText(ZPF.XF.Basics.Current.FileIO.CleanPath($@"{MainViewModel.Current.DataFolder}Interventions\{tmpInter.PK}.json"));
            //   var p = JsonSerializer.Deserialize<Intervention_Params>(json);

            //   if (p != null)
            //   {
            //      tmpInter.SetParams(p);
            //   };
            //}
            //catch (Exception ex)
            //{
      ...
      public async Task SyncDataWithWeb(string login)
      {
         ...
         //ToDo: SyncArticles();
         //if (string.IsNullOrEmpty(SyncViewModel.Current.LastError)) await SyncArticles();

         //DoIt.OnMainThread(() =>
         //{
         //   if (string.IsNullOrEmpty(SyncViewModel.Current.LastError))
         //   {
         //      BackboneViewModel.Current.BusySubTitle = $"Articles {MainViewModel.Current.Articles.Count} \n";
         //   }
         //   else
         //   {
         //      return;
         ...
         //if (string.IsNullOrEmpty(SyncViewModel.Current.LastError)) await SyncFamilies();

         //DoIt.OnMainThread(() =>
         //{
         //   if (string.IsNullOrEmpty(SyncViewModel.Current.LastError))
         //   {
         //      BackboneViewModel.Current.BusySubTitle = $"Families {MainViewModel.Current.Families.Count} \n";
         //   }
         //   else
         //   {
         //      return;
